...
def play(strategy0, strategy1,
    ...
    while score0 < goal and score1 < goal:
        ...
        if who == 0:
            ...
            swap = swine_swap(score0)
        else:
            num_rolls = strategy1(score1, score0)
            score1 += take_turn(num_rolls, score0, dice)
            swap = swine_swap(score1)
        if swap:
...
def swine_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if swine_swap(new_score):
        new_score = opponent_score
    return 0 if new_score - old_score >= threshold else num_rolls
    # END PROBLEM 11
