...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def is_in_paragraph(paragraph):
        words = split(lower(remove_punctuation(paragraph)))
        for word in topic:
            if word in words:
                return True
        return False
    return is_in_paragraph
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if not typed_words and not source_words:
        ...
        count, total = 0, len(typed_words)
        for i in range(min(total, len(source_words))):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    scores = [diff_function(typed_word, word, limit) for word in word_list]
    min_score, min_word = 233333, None
    for i in range(len(word_list)):
        if scores[i] < min_score and scores[i] <= limit:
            min_score, min_word = scores[i], word_list[i]
    return min_word or typed_word  # if min_word is None, return typed_word
    # END PROBLEM 5
...
def sphinx_fixes(typed, source, limit):
    ...
    if limit < 0:
        ...
        return sphinx_fixes(typed[1:], source[1:], limit - diff) + diff
    # END PROBLEM 6
...
def time_per_word(words, times_per_player):
    ...
    n = len(words)
    t = times_per_player
    return game(words, [[p[i + 1] - p[i] for i in range(n)] for p in t])
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for word_index in word_indices:
        fastest_time, fastest_index = 1145141919810, -1
        for player_index in player_indices:
            t = time(game, player_index, word_index)
            if t < fastest_time:
                fastest_time, fastest_index = t, player_index
        results[fastest_index] += [get_word(game, word_index)]
    return results
    # END PROBLEM 10
