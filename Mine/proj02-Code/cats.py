...
def about(topic):
    ...
    def check(paragraphs):
        for obj in split(paragraphs):
            for x in topic:
                if remove_punctuation(lower(obj)) == x:
                    return True
        return False
    return check
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words),len(source_words))):
        if typed_words[i] == source_words[i]:
            cor = cor + 1
    return cor / len(source_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for elm in word_list:
        if diff_function(typed_word,elm,limit) < Min:
            ans = elm
            Min = diff_function(typed_word,elm,limit)
        if elm == typed_word:
            return typed_word
...
def sphinx_fixes(typed, source, limit):
    ...
    if len(source) > len(typed):
        res = res + len(source) - len(typed)
    else :
        res = res + len(typed) - len(source)  
    return res
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 7
    def check_next(start, goal, limit, have):
        if have > limit:
            return limit + 1
        ...
        # END
            ...
            return check_next(start[1:], goal[1:], limit, have)
        else:
            add = check_next(start, goal[1:], limit, have + 1) + 1
            remove = check_next(start[1:], goal, limit, have + 1) + 1
            substitute = check_next(start[1:], goal[1:], limit, have + 1) + 1
        # BEGIN
            return min(add, remove, substitute, limit + 1)
    return check_next(start, goal, limit, 0)
        # END
    # END PROBLEM 7
...
def report_progress(typed, prompt, user_id, upload):
    ...
    while(i < min(len(typed), len(prompt))):
        if typed[i] != prompt[i]:
            ...
            i = i + 1
    rat = i / len(prompt)
    upload({'id':user_id, 'progress':rat})
    return rat
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    #res_times=[[] for _ in times_per_player]
    res_time = []
    for player in range(0, len(times_per_player)):
        res_time.append([])
        for i in range(1, len(times_per_player[player])):
            res_time[player].append(times_per_player[player][i] - times_per_player[player][i - 1])
    return game(words, res_time)    
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for player in player_indices:
        res_list.append([])
        for now_word in word_indices:
            flag = 1
            now_time = times[player][now_word]
            for other_player in player_indices:
                if other_player == player:
                    continue
                other_time = times[other_player][now_word]
                if other_player < player and other_time <= now_time:
                    flag = 0
                    break
                elif other_player > player and other_time < now_time:
                    flag = 0
                    break
            if flag == 1:
                res_list[player].append(words[now_word])
