...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN PROBLEM 8
            other = place.ant
            if not isinstance(self, ContainerAnt) and not isinstance(other, ContainerAnt):
                assert place.ant is None, 'Two ants in {0}'.format(place)

            if isinstance(other, ContainerAnt) and ContainerAnt.can_contain(other, self): #?
                #print('123')
                other.store_ant(self)

            elif isinstance(self, ContainerAnt) and ContainerAnt.can_contain(self, other): #?
                #print('qer')
                place.ant = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        tar = random_bee(tmplace.bees)
        if tar and min_range <= distance <= max_range:
            return tar
        while tmplace.entrance:
            tmplace = tmplace.entrance
            distance += 1
            if tmplace.is_hive == True:
                return None
            else:
                tar = random_bee(tmplace.bees)
                if tar and min_range <= distance <= max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        tmpdamage = amount
        if self.place == None:
            tmpdamage += self.damage
        bees = tmplace.bees[:]
        for bee in bees:
            bee.reduce_health(tmpdamage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN PROBLEM 12
        if gamestate.havequeen:
            return
        

        if ScubaThrower.construct(gamestate):
            gamestate.havequeen = True
            return cls()
        else:
            return
        # END PROBLEM 12
    ...
    def action(self, gamestate):
        ...
        tmplace = self.place.exit
        while tmplace:
            if tmplace.ant == None:
                tmplace = tmplace.exit
                continue
            ant = tmplace.ant
